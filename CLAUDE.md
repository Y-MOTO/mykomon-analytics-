...
### 問題6：「📨 人事経営相談に送る」がAIレポート本文を含めていなかった（2026-04-27 修正）

**症状**：「📨 人事経営相談に送る」を押すと集計統計だけが入力欄にセットされ、Claude が生成したレポート本文が含まれない。  
**原因**：`gap` dict の構築時に `ai_report`（`st.session_state.ai_report`）を含めていなかった。`format_gap_prefill` も統計データしか出力しない実装だった。  
**対処**：

```python
# gap dict に ai_report を追加（Tab5 ボタン押下時）
gap = {
    ...
    "ai_report": st.session_state.get("ai_report", ""),  # 追加
}

# format_gap_prefill でレポート本文を先頭に出力
def format_gap_prefill(gap: dict) -> str:
    lines = []
    if gap.get("ai_report"):
        lines += ["【AI分析レポート（Claude生成）】", gap["ai_report"], "\n---"]
    # 以降は統計データ → 所長補足欄
```
...
> AIレポートを生成せずにボタンを押した場合は、レポート部分が省略されて統計データのみがセットされる（正常動作）。
